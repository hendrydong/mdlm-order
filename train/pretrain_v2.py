...
def iter_fixed_chunks(
    ...
    for item in iterator:
        ...
        while len(buf_ids) >= chunk_size:
            ...
            out_orders = [buf_orders.popleft() for _ in range(chunk_size)]
            yield {"input_ids": out_ids, "segment_ids": out_segs, "order_ids": out_orders}

    # 处理尾巴
    if not drop_last and len(buf_ids) > 0:
        out_ids = list(buf_ids)
        out_segs = list(buf_segs)
        out_orders = list(buf_orders)
        yield {"input_ids": out_ids, "segment_ids": out_segs, "order_ids": out_orders}
...
def prepare_pretrain_packed_concat(ds, tokenizer, chunk_size, add_eos=True, eos_fallback=1):
    ...
    def tok_fn(examples):
        ids_list = examples["input_ids"][0]
        # 为每条样本分配一个 doc 段号，整条样本同一段号
        seg_list = [torch.randint(1024,[]).item()] * len(ids_list) 
        return {"input_ids": ids_list, "segment_ids": seg_list, "order_ids": examples["decode_orders"]}

    tokenized = ds.map(tok_fn, remove_columns=ds.column_names, num_proc=512)

    # 使用生成器分批处理，避免一次性加载所有数据到内存
    features = {
        "input_ids": Sequence(Value("int32")),
        "segment_ids": Sequence(Value("int32")),
        "order_ids": Sequence(Value("int32")),
    }
...
def collect_training_data(
    ...
    order_ids=None,         # [B, L] LongTensor，可选
    K = None,
):
...
def main():
    ...
    def collate_build_batch(batch, base_bias=base_bias, K=config.training.block_size):
        ...
        seg_ids   = torch.tensor([ex["segment_ids"] for ex in batch],
                                dtype=torch.long, device=accelerator.device)    # [B, L]
        order_ids = torch.tensor([ex["order_ids"] for ex in batch],
                                dtype=torch.long, device=accelerator.device)    # [B, L]
